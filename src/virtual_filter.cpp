...
{
	virtual_filter_data *data = (virtual_filter_data *)bzalloc(
		sizeof(struct virtual_filter_data));
				 ...
				 void *data)
{
	...
	if (base_width > 0 && base_height > 0) {
		...
		filter->width = 0;
		filter->height = 0;
		interval = (uint64_t)ovi.fps_den * 1000000000ULL /
			   (uint64_t)ovi.fps_num;
		filter->active = shared_queue_create(
			&filter->video_queue, filter->mode, AV_PIX_FMT_BGRA,
			filter->base_width, filter->base_height, interval,
			filter->delay + 10);
	} else {
...
static obs_properties_t *virtual_filter_properties(void *data)
{
	...
	obs_property_list_add_int(cb, "OBS-Camera5", ModeVideo5); // ★ added
	obs_property_list_add_int(cb, "OBS-Camera6", ModeVideo6); // ★ added
	obs_property_list_add_int(cb, "OBS-Camera7", ModeVideo7); // ★ added
	obs_property_list_add_int(cb, "OBS-Camera8", ModeVideo8); // ★ added
